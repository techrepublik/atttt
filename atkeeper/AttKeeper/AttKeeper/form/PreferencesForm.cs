...
namespace AttKeeper.form
{
    public partial class PreferencesForm : Form
    {
        ...
        private void companyBindingSource_CurrentChanged(object sender, EventArgs e)
        {
            ...
            textBoxSchoolName.Text = ((Company) companyBindingSource.Current).CompanyName;
            textBox1.Text = ((Company) companyBindingSource.Current).CompanyAddress;
        }
        ...
        private void InitConfig()
        {
            ...
            checkBoxWeekend.Checked = Convert.ToBoolean(ConfigurationManager.AppSettings["Weekend"]);
        ...
        {
            if (String.IsNullOrEmpty(textBoxSchoolName.Text)) return;
            Graphics graphics = null;
            Image image = null;
            var filePath = Directory.GetCurrentDirectory() + @"\images\rpt_w.jpg";
            var filePathOut = Directory.GetCurrentDirectory() + @"\images\rpt_w_out.jpg";
            image = Image.FromFile(filePath);
            graphics = Graphics.FromImage(image);
            var font = new Font(@"Arial", 64.0f);
            var lSchool = textBoxSchoolName.Text.Trim().Length;
            var lAddress1 = textBox1.Text.Trim().Length;
            var lAddress2 = textBox2.Text.Trim().Length;
            var lRight = (int)((590 - lSchool)/3);
            var lLeft = (int)((590 - lAddress1)/7);
            var lBottom = (int)((590 - lAddress2) / 2);
            //var point = new PointF(100, 500);
            var point1 = new PointF(-450, 500);
            var point2 = new PointF(-530, 566);
            var point3 = new PointF(-490, 628);
            graphics.RotateTransform(-45);
            //graphics.DrawString(textBoxSchoolName.Text, font, Brushes.LightGray, lRight, 650);
            graphics.DrawString(@"Department of Education", font, Brushes.LightGray, point1);
            graphics.DrawString(textBoxSchoolName.Text, font, Brushes.LightGray, point2);
            graphics.DrawString(textBox1.Text, font, Brushes.LightGray, point3);
            image.Save(filePathOut);
            pictureBox1.Image = new Bitmap(filePathOut);
        }

        private void checkBoxWeekend_Validating(object sender, CancelEventArgs e)
        {
            try
            {
                if (checkBoxWeekend.CheckState == CheckState.Checked)
                    _config.AppSettings.Settings["Weekend"].Value = @"True";
                else
                {
                    _config.AppSettings.Settings["Weekend"].Value = @"False";
                }
                _config.Save(ConfigurationSaveMode.Modified);
                ConfigurationManager.RefreshSection("appSettings");
            }
            ...
            {
                MessageBox.Show(@"Cannot update Weekend status due to Windows file and folder security restriction.
                                 Kindly contact administrator for assistance.", @"Security Resctricion",
                     MessageBoxButtons.OK, MessageBoxIcon.Information);
                Console.WriteLine(ex.Message);
            }
