...
namespace AttKeeper.core.dtr
{
    ...
    {
        #region LoadDTR
        public static List<DTR> LoadDTRViaDTR(Empoyee enrollee, int iMonth, int iYear, List<DTR> lDTRs, out string sSetting, bool bWeekend)
        {
        ...
        public static List<Machine> RemoveDuplicateUsb(List<DatData> listLogs, int instanceId, int device)
        {
            var lMacDB = MachineManager.GetAll();
            List<DatData> listMacDB = null;
            switch (device)
            {
                case 1:
                    listMacDB = lMacDB.Select(item => new DatData
                    {
            ...
            return (from log in listLogs
                ...
                select new Machine
                {
                    MachineInsId = instanceId, EnrolleeNo = Convert.ToInt32(log.Pin.Trim()).ToString(), IYear = Convert.ToDateTime(log.AttTime).Year, IMonth = Convert.ToDateTime(log.AttTime).Month, IDay = Convert.ToDateTime(log.AttTime).Day, IHour = Convert.ToDateTime(log.AttTime).Hour, IMin = Convert.ToDateTime(log.AttTime).Minute, ISec = Convert.ToDateTime(log.AttTime).Second
                }).ToList();
        }
